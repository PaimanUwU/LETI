...
import { 
  Save, FileText, Eye, Edit3, RotateCcw, 
...
const PAGES: PageConfig[] = [
  { id: "ai", title: "AI Engine Page", description: "Technical parameters & forecasting intervals", icon: FileText },
  { id: "data", title: "Data Architecture", description: "Primary ingestion & crowdsourcing reports", icon: FileText },
  { id: "docs", title: "Project Blueprint", description: "Institutional guidelines, objectives & team matrix", icon: FileText },
];
...
export default function Setting() {
  ...
  useEffect(() => {
    const fetchPageContent = async () => {
      ...
      try {
        ...
        const res = await fetch(`http://localhost:8000/api/settings/${selectedPage}`);
        if (res.ok) {
        ...
        console.warn("Backend API not reachable or failed, attempting local cache/static files.", e);
      }
      ...
      try {
        ...
        if (res.ok) {
          ...
          setStatus({ type: "error", message: `Failed to fetch original markdown file /markdown/${selectedPage}.md` });
        }
      } catch (e) {
        setStatus({ type: "error", message: `Network error loading page content: ${(e as Error).message}` });
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatus({ type: "success", message: `Settings for "${PAGES.find(p => p.id === selectedPage)?.title}" saved successfully to the backend!` });
        setSaving(false);
        return;
      } else {
      ...
      console.warn("Could not save to backend. Falling back to local storage cache.", e);
      ...
      try {
        ...
        setStatus({ type: "error", message: `Failed to save changes: ${(err as Error).message}` });
      } finally {
        setSaving(false);
      }
    }
  ...
  const handleReset = async () => {
    if (!window.confirm(`Are you sure you want to reset the "${PAGES.find(p => p.id === selectedPage)?.title}" content back to default? All unsaved edits will be lost.`)) {
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStatus({ type: "success", message: "Content has been reset to system defaults!" });
      } else {
        setStatus({ type: "error", message: "Failed to retrieve default markdown file." });
      }
    } catch (e) {
      setStatus({ type: "error", message: `Error resetting content: ${(e as Error).message}` });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-muted-foreground">Manage and edit the content of your public Info pages dynamically using Markdown.</p>
        </div>
        <Badge variant="secondary" className="flex items-center gap-1.5 py-1 px-3 bg-primary/10 text-primary border-none">
            ...
            >
              <CardHeader className="p-5 flex flex-row items-center gap-4 space-y-0">
                ...
                <div className="flex-1">
                  ...
                  <CardDescription className="text-xs line-clamp-1">{page.description}</CardDescription>
                </div>
              </CardHeader>
            </Card>
          );
          ...
          <CardHeader className="border-b border-slate-200 dark:border-slate-800 py-4 px-6 flex flex-row items-center justify-between space-y-0 flex-wrap gap-4">
            <div className="flex items-center gap-4">
              ...
              <TabsList className="bg-muted">
                <TabsTrigger value="edit" className="flex items-center gap-1.5 text-xs">
          ...
          <CardContent className="p-0">
            {loading ? (
              <div className="flex flex-col items-center justify-center py-20 gap-3">
                ...
                <span className="text-sm text-muted-foreground font-medium">Loading page content...</span>
              </div>
            ) : (
              <>
                <TabsContent value="edit" className="m-0 p-6">
                  <div className="flex flex-col gap-2">
                    <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Markdown Source</label>
                    <textarea
                      value={markdown}
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Supports standard markdown formatting: **bold**, *italics*, # headers, - lists, tables, and links.
                    </p>
                  </div>
                </TabsContent>

                <TabsContent value="preview" className="m-0 p-6 bg-slate-50/50 dark:bg-slate-950/20">
